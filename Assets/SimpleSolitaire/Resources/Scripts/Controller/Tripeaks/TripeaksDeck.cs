    ...
    public class TripeaksDeck : Deck
    {
        ...
        public override void UpdateCardsPosition(bool firstTime)
        {
            ...
            for (int i = 0; i < CardsArray.Count; i++)
            {
                ...
                {
                    var packHorizontalSpace = CardLogicComponent.GetSpaceFromDictionary(DeckSpacesTypes.DECK_PACK_HORIZONTAL_RIGHT);
        ...
        public override void UpdateDraggableStatus()
        {
            for (int i = 0; i < CardsArray.Count; i++)
            {
                ...
                {
                    bool isOverlapped = _tripeaksCardLogic.CheckOverlaping(card) && !card.OverlapsAlreadyInWaste(_tripeaksCardLogic.IdsInWaste);
